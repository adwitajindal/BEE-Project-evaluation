...
import { MdDelete, MdPerson, MdStorage, MdAdminPanelSettings, MdSearch, MdRefresh } from "react-icons/md";
...
const AdminDashboard = () => {
  ...
  const getAllUsers = async () => {
    ...
    try {
      ...
      if (response.data && response.data.users) {
        ...
        const total = response.data.users.reduce((acc, user) => acc + (user.storageUsed || 0), 0);
  ...
  return (
    <>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-gradient-to-r from-cyan-50 to-blue-50 p-6 rounded-lg shadow-sm">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-sm text-slate-500 mb-1">Total Storage Used</p>
                  <h3 className="text-3xl font-bold text-slate-800">{formatBytes(totalStorage)}</h3>
                </div>
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <h3 className="text-3xl font-bold text-slate-800">
                    {users.reduce((acc, user) => acc + (user.storiesCount || 0), 0)}
                  </h3>
                </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full bg-white rounded-lg overflow-hidden">
              <thead className="bg-gray-50 text-gray-600 text-sm">
                <tr>
                  <th className="py-3 px-4 text-left cursor-pointer" onClick={() => requestSort('fullName')}>
                    User {getSortIndicator('fullName')}
                  </th>
                  <th className="py-3 px-4 text-left cursor-pointer" onClick={() => requestSort('email')}>
                    Email {getSortIndicator('email')}
                  </th>
                  <th className="py-3 px-4 text-left cursor-pointer" onClick={() => requestSort('storiesCount')}>
                    Stories {getSortIndicator('storiesCount')}
                  </th>
                  <th className="py-3 px-4 text-left cursor-pointer" onClick={() => requestSort('storageUsed')}>
                    Storage Used {getSortIndicator('storageUsed')}
                  </th>
                  <th className="py-3 px-4 text-left cursor-pointer" onClick={() => requestSort('createdAt')}>
                ...
                {loading ? (
                  ...
                  filteredUsers.map((user) => (
                    <tr key={user.id || user._id} className="hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4 text-gray-500">{user.storiesCount || 0}</td>
                      <td className="py-3 px-4 text-gray-500">{formatBytes(user.storageUsed || 0)}</td>
      ...
      >
        <div className="p-4">
          <h2 className="text-xl font-bold text-gray-800 mb-4">Confirm Account Deletion</h2>
          <p className="mb-6 text-gray-600">
            Are you sure you want to delete the account for <span className="font-semibold">{userToDelete?.fullName}</span>? 
            This will permanently remove all their data including stories and images.
          </p>
